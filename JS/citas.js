...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function cargarMisCitas(idCliente) {
    ...
    const usuario = JSON.parse(localStorage.getItem("usuario"))
    const isBarbero = usuario && usuario.rol === 2 // rol 2 = barbero
    const endpoint = isBarbero ? "../php/citas_barbero_get.php" : `../php/citas_cliente_get.php?id_cliente=${idCliente}`

    console.log("[v0] Cargando citas. isBarbero:", isBarbero, "Endpoint:", endpoint)
    ...
    fetch(endpoint)
      ...
      .then((data) => {
        ...
        if (citas.length > 0) {
          listaCitas.innerHTML = citas
            .map(
              (cita) => `
                          <div class="cita-card ${cita.estado}">
                              <div class="cita-info">
                                  <h4>${cita.servicio}</h4>
                                  <p><strong>${isBarbero ? "Cliente" : "Barbero"}:</strong> ${isBarbero ? cita.cliente_nombre : cita.nombre_barbero}</p>
                                  <p><strong>Fecha:</strong> ${formatearFecha(cita.fecha_hora ? cita.fecha_hora.split(" ")[0] : cita.fecha)}</p>
                                  <p><strong>Hora:</strong> ${formatearHora(cita.fecha_hora ? cita.fecha_hora.split(" ")[1] : cita.hora)}</p>
                              </div>
